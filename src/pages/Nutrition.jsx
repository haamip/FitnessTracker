...
export default function Nutrition() {
  ...
  useEffect(() => {
    ...
    async function loadMeals() {
      ...
      if (!active) return;
      if (error) {
        setMessage(`Cloud sync issue: ${error.message}`);
        return;
      }
      ...
      const otherDays = NutritionRepository.getAll().filter((meal) => meal.date !== today);
  ...
  const totals = useMemo(
    () => meals.reduce(
      (sum, meal) => ({
        calories: sum.calories + toNumber(meal.calories),
        protein: sum.protein + toNumber(meal.protein),
      }),
  ...
  async function handleSubmit(event) {
    ...
    const localMeal = {
      ...
      date: today,
      type: form.type,
      name: form.name.trim() || "Quick meal",
      calories: roundDisplay(form.calories),
      protein: roundDisplay(form.protein),
      carbs: 0,
      fats: 0,
    ...
    if (isSupabaseConfigured && supabase) {
      ...
      if (userId) {
        const { data, error } = await supabase
          ...
          .insert({
            user_id: userId,
            entry_date: localMeal.date,
            meal_type: localMeal.type,
            name: localMeal.name,
            calories: localMeal.calories,
            protein_g: localMeal.protein,
            carbs_g: localMeal.carbs,
            fats_g: localMeal.fats,
          })
          .select()
          .single();

        if (error) {
          setMessage(`Meal was not saved: ${error.message}`);
          setSaving(false);
          return;
        }

        const savedMeal = fromDatabase(data);
        const nextMeals = [savedMeal, ...meals];
        setMeals(nextMeals);
        NutritionRepository.add(savedMeal);
        setForm(initialForm);
        setSaving(false);
        return;
      }
    ...
    NutritionRepository.add(localMeal);
  ...
  async function deleteMeal(id) {
    ...
    if (isSupabaseConfigured && supabase) {
      ...
      if (error) {
        setMessage(`Meal was not deleted: ${error.message}`);
        return;
      }
    }
  ...
  return <motion.div className="screen checkin-v4" initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} transition={{duration:0.25}}>
    ...
    <section className="tf-food-summary">
      <article className="tf-food-target"><div className="tf-food-target__top"><div><Flame size={20}/><span>Calories</span></div><strong>{roundDisplay(totals.calories)} / {CALORIE_TARGET}</strong></div><div className="progress-line"><span style={{width:`${calorieProgress}%`}}/></div></article>
      <article className="tf-food-target"><div className="tf-food-target__top"><div><Beef size={20}/><span>Protein</span></div><strong>{roundDisplay(totals.protein)} / {PROTEIN_TARGET}g</strong></div><div className="progress-line"><span style={{width:`${proteinProgress}%`}}/></div></article>
    </section>
